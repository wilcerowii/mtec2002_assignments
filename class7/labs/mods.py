...
mods.py
===
A module is just a text file with Python code in it.  Modules can be used to store 
reusable code, such as functions, classes.  You've actually used, and
even created, modules before!  

You can use a module by importing it.  Here are a couple of ways of 
importing modules (one should look familiar to you!): 
* import my_module
* from my_module import *

Generally, it's best to use the "import my_module" style to preserve
namespaces.  That is... if you import *, you can accidentally 
override code in the module or code that you've written.  When you 
import a moudle by using "my_module":
* a namespace is created with that module's name
* variables, functions and classes can be accessed in that module by
using the module name and the dot (.) operator

Examples:
1. import math 				# importing a built-in module named math
2. print math.pi			# print out the variable named pi in the math module
3. x = math.ceil(3.7)		# call the function ceil() that's definited 
the math module; assign it to x
...
* random -  http://docs.python.org/library/math.html
	* random.randint(a, b) - returns a random number that greater than 
...
def get_creatures = ["unicorn", "dragon","yeti"]
index = random.randith(0,2)
print "%s, %s" % (index,creatures[index])
### math
